...
Cloudflare Worker 上运行的 MCP (Model Context Protocol) 图像生成服务，通过 987xyz 网关调用 `gpt-image-2` 模型。

## 线上地址

```
https://image.qdp.qzz.io
```
...
```bash
curl -X POST https://image.qdp.qzz.io/mcp \
...
```bash
curl https://image.qdp.qzz.io/health
```

### Legacy REST（向后兼容）

```bash
curl -X POST https://image.qdp.qzz.io/tools/generate_image \
  -H 'Content-Type: application/json' \
...
| 变量 | 说明 |
|------|------|
| `API_KEY` | 987xyz API Key（在 CF Worker Settings → Secrets 中配置） |
...
Worker 名称：`image-mcp-worker`，路由：`image.qdp.qzz.io/*`
