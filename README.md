...
标准 MCP Streamable HTTP / JSON-RPC 2.0。无需额外认证 header。所有请求为 `POST` + `Content-Type: application/json`。
...
```bash
curl -X POST <MCP_URL>/tools/generate_image \
  -H 'Content-Type: application/json' \
